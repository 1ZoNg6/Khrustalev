-- =============================================
-- Quick Premium Test - Показывает результаты
-- =============================================
...
INSERT INTO employee_metrics (
    ...
    period_start,
    period_end,
    task_completion_frequency,      -- 4
    tasks_not_completed_on_time,    -- 1
    tasks_completed_on_time,        -- 2
    total_contract_value,           -- 450,000
    number_of_delays               -- 0
)
SELECT 
    ...
    0       -- Опоздания
WHERE NOT EXISTS (
    SELECT 1 FROM employee_metrics 
...
INSERT INTO employee_metrics (
    ...
    period_start,
    period_end,
    task_completion_frequency,      -- 3
    tasks_not_completed_on_time,    -- 2
    tasks_completed_on_time,        -- 1
    total_contract_value,           -- 300,000
    number_of_delays               -- 1
)
SELECT 
...
WHERE NOT EXISTS (
    SELECT 1 FROM employee_metrics 
    WHERE period_start = '2024-01-01' AND period_end = '2024-01-31'
    AND user_id != (SELECT id FROM profiles WHERE role = 'Работник' LIMIT 1)
);
